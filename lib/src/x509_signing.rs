...
//  [GNUPG:] GOODSIG <fingerprint> <subject>
//  [GNUPG:] EXPKEYSIG <fingerprint> <subject>
//  [GNUPG:] BADSIG <fingerprint> <subject>
// in the output from --status-fd=1
// Assume signature is invalid if none of the above was found
fn parse_x509_verify_output(
    output: &[u8],
...
fn run_sign_command(command: &mut Command, input: &[u8]) -> Result<Vec<u8>, X509Error> {
    tracing::info!(?command, "running gpgsm signing command");
    let process = command.stderr(Stdio::piped()).spawn()?;
    let write_result = process.stdin.as_ref().unwrap().write_all(input);
    let output = process.wait_with_output()?;
    tracing::info!(?command, ?output.status, "gpgsm signing command exited");
    if output.status.success() {
        write_result?;
        Ok(output.stdout)
    } else {
        Err(X509Error::Command {
...
fn run_verify_command(command: &mut Command, input: &[u8]) -> Result<Vec<u8>, X509Error> {
    tracing::info!(?command, "running gpgsm signing command");
    ...
    match write_result {
        Ok(()) => Ok(output.stdout),
        // If the signature format is invalid, gpgsm will terminate early. Writing
...
#[derive(Debug, Error)]
pub enum X509Error {
    #[error("gpgsm failed with {exit_status}:\n{stderr}")]
    Command {
        exit_status: ExitStatus,
        stderr: String,
    },
    #[error("Failed to run gpgsm")]
    Io(#[from] std::io::Error),
}

impl From<X509Error> for SignError {
...
impl X509Backend {
    ...
    pub fn from_settings(settings: &UserSettings) -> Result<Self, ConfigGetError> {
        let program = settings.get_string("signing.backends.x509.program")?;
        let allow_expired_keys = settings.get_bool("signing.backends.x509.allow-expired-keys")?;
...
impl SigningBackend for X509Backend {
    fn name(&self) -> &str {
        "x509"
    }
    ...
    fn verify(&self, data: &[u8], signature: &[u8]) -> Result<Verification, SignError> {
        let mut signature_file = tempfile::Builder::new()
            .prefix(".jj-x509-sig-tmp-")
            .tempfile()
            .map_err(X509Error::Io)?;
        signature_file.write_all(signature).map_err(X509Error::Io)?;
        signature_file.flush().map_err(X509Error::Io)?;
        ...
        parse_x509_verify_output(&output, self.allow_expired_keys)
    }
}
...
mod tests {
    ...
    #[test]
    fn x509_verify_invalid_signature_format() {
        use assert_matches::assert_matches;
        assert_matches!(
            parse_x509_verify_output(b"", true),
            Err(SignError::InvalidSignatureFormat)
        );
    }

    #[test]
    fn x509_verify_bad_signature() {
        assert_eq!(
            parse_x509_verify_output(b"[GNUPG:] BADSIG 123 456", true).unwrap(),
            Verification::new(SigStatus::Bad, Some("123".into()), Some("456".into()))
        );
    }

    #[test]
    fn x509_verify_good_signature() {
        assert_eq!(
            parse_x509_verify_output(b"[GNUPG:] GOODSIG 123 456", true).unwrap(),
            Verification::new(SigStatus::Good, Some("123".into()), Some("456".into()))
        );
    }

    #[test]
    fn x509_verify_expired_signature() {
        assert_eq!(
            parse_x509_verify_output(b"[GNUPG:] EXPKEYSIG 123 456", true).unwrap(),
            Verification::new(SigStatus::Good, Some("123".into()), Some("456".into()))
        );

        assert_eq!(
            parse_x509_verify_output(b"[GNUPG:] EXPKEYSIG 123 456", false).unwrap(),
            Verification::new(SigStatus::Bad, Some("123".into()), Some("456".into()))
        );
    }
}
