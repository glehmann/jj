...
pub(crate) fn cmd_squash(
    ...
    if !args.from.is_empty() || args.into.is_some() || use_new_flags {
        ...
        if use_new_flags {
            ...
            let destination = if let Some(into) = &args.into {
                workspace_command.resolve_single_rev(ui, into)?
            } else {
    ...
    let destination = if let Some(commit) = pre_existing_destination {
        ...
        tx.base_workspace_helper()
            .check_rewritable(sources.iter().ids().chain(child_ids.iter()))?;
        ...
        for child_id in &child_ids {
            let child_commit = tx.repo().store().get_commit(child_id)?;
            let new_parent_ids = child_commit
                .parent_ids()
                .iter()
                .filter(|id| !&parent_ids.contains(id))
                .cloned()
                .chain(std::iter::once(commit.id().clone()))
                .collect_vec();
            let new_child_commit =
                rebase_commit(tx.repo_mut(), child_commit, new_parent_ids).block_on()?;
            if let Some(source_commit) = sources
                .iter()
                .position(|source| source.id() == child_id)
                .and_then(|index| sources.get_mut(index))
            {
                *source_commit = new_child_commit;
            }
        }
        tx.repo_mut()
            .transform_descendants(child_ids, async |rewriter| {
                let old_commit_id = rewriter.old_commit().id().clone();
                let new_commit = rewriter.rebase().await?.write()?;
                if let Some(source_commit) = sources
                    .iter()
                    .position(|source| source.id() == &old_commit_id)
                    .and_then(|index| sources.get_mut(index))
                {
    ...
    if let Some(squashed) = rewrite::squash_commits(
        ...
        let new_description = match description {
            ...
            SquashedDescription::Combine => {
                ...
                // TODO: chain the let expression, once our MSRV is at least 1.88
                let description = try_combine_messages(abandoned_commits, &destination);
                if !use_new_flags && description.is_some() {
                    let description = description.unwrap();
                    if description.is_empty() {
                        description
                    } else {
                        commit_builder.set_description(description);
                        add_config_trailers_with_extras(
                            ui,
                            &tx,
                            &commit_builder,
                            &co_author_trailers,
